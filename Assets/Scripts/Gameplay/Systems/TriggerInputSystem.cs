        ...
        foreach (var entityProj in projectileFilter)
        {
            ...
            if (projectile.modelTransform.gameObject == _sender)
            {
                foreach (var entityEnemy in enemyFilter)
                {
                    ...
                    if (enemy.modelTransform.gameObject == _collision)
                    {
                        _damage = projectileConfig.Weapons[0].Damage;
                        Debug.Log("damage to enemy");
                        if (enemyConfig.HP == 0)
                        {
                            enemyConfig.HP = enemyConfig.Config.HP;
                        }
                        enemyConfig.HP -= _damage;
                        if (enemyConfig.HP <= 0)
                        {
                            _enemyCollision.SetActive(false);
                            Object.Instantiate(enemyConfig.Config.ExperienceCrystal, _enemyCollision.transform.position,
