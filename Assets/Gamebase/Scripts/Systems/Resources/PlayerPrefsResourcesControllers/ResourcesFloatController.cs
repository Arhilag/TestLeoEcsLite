...
namespace Gamebase
{
    ...
    /// </summary>
    public class ResourcesFloatController
    {
        public ResourcesFloatController(Action<ResourceType, PlayerPrefsResourceType> onRegister)
        {
            _onRegister = onRegister;
        }
        
        private readonly Dictionary<ResourceType, PlayerPrefsFloatResource> _resources =
            new Dictionary<ResourceType, PlayerPrefsFloatResource>();
        private readonly Action<ResourceType, PlayerPrefsResourceType> _onRegister;

        internal void Register(PlayerPrefsFloatResource resource)
        {
            if (resource.PlayerPrefsType != PlayerPrefsResourceType.Float)
            {
                DebugSystem.LogError($"[ResourcesSystem] - The resource type {resource.Type} cannot be registered as an Float resource");
                return;
            }
        ...
        public void Add(ResourceType resourceType, float count)
        {
        ...
        public void Take(ResourceType resourceType, float count)
        {
            if (!ResourceFound(resourceType)) return;
        ...
        public void Set(ResourceType resourceType, float count)
        {
            if (!ResourceFound(resourceType)) return;
        ...
        public float Get(ResourceType resourceType)
        {
        ...
        public async UniTask Subscribe(ResourceType resourceType, Action<float> action)
        {
        ...
        public void Unsubscribe(ResourceType resourceType, Action<float> action)
        {
            if (!ResourceFound(resourceType)) return;
            _resources[resourceType].OnResourceCountChanged -= action;
        }
