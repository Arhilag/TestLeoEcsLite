...
namespace Doozy.Engine.UI.Nodes
{
    ...
    {
        private static GlobalEventsSystem _globalEventsSystem => GamebaseSystems.Instance.GlobalEventsSystem;
        ...
        private void AddListeners()
        {
            ...
            _globalEventsSystem.SubscribeToAll(OnGlobalEvent);
        }
        ...
        private void OnGlobalEvent(GlobalEventType type)
        {
            if (ActiveGraph != null && !ActiveGraph.Enabled) return;
            if (OutputSockets == null || OutputSockets.Count == 0) return;
            ...
            foreach (Socket socket in OutputSockets)
            {
                if (!socket.IsConnected) continue;
                UIConnection value = UIConnection.GetValue(socket);
                if (value.Trigger != UIConnectionTrigger.GlobalEvent) continue;
                if (value.GlobalEvent != type) continue;
                ActivateOutputSocketInputNode(socket);
                break;
            }
        }
